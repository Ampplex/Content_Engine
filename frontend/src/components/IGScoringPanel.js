      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200/60 overflow-hidden h-full flex flex-col">
        <div className="px-5 py-4 border-b border-slate-100">
          <h2 className="font-semibold text-sm flex items-center gap-2 text-slate-800">
            <BarChart3 className="w-4 h-4 text-pink-500" /> Engagement Score
          </h2>
        ...
        <div className="p-5 flex-grow">
          {scores ? (
            <>
              ...
              <div className="text-center mb-5">
                ...
                <div className="relative inline-flex items-center justify-center">
                  <svg className="w-32 h-32 -rotate-90" viewBox="0 0 120 120">
                    ...
                    <circle
                      ...
                      stroke={scores.final >= 0.75 ? '#ec4899' : '#f59e0b'}
                ...
                <p className={`text-sm font-bold mt-2 ${
                  scores.final >= 0.85 ? 'text-emerald-600'
                  : scores.final >= 0.75 ? 'text-pink-600'
                  : 'text-amber-600'
                }`}>
                  {scores.final >= 0.85 ? '🔥 High Viral Potential'
                   : scores.final >= 0.75 ? '✅ Good Engagement'
                   : '⚠️ Needs Improvement'}
                </p>
              </div>
              ...
              <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-100 mb-4">
                ...
                <p className="text-xs text-slate-600 font-mono leading-loose">
                  (0.5 × {(scores.ml * 100).toFixed(1)})<br />
                  + (0.3 × {(scores.llm * 100).toFixed(1)})<br />
                  + (0.2 × {(scores.heuristic * 100).toFixed(1)})<br />
              ...
              {scores.ml_top_importances && Object.keys(scores.ml_top_importances).length > 0 && (
                <div className="bg-pink-50/60 rounded-xl p-4 border border-pink-100">
                  <p className="text-[10px] font-bold text-pink-500 uppercase mb-3">Top Caption Features</p>
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-200 gap-3 py-16">
              ...
              <p className="text-sm font-medium text-slate-300">Scores appear here</p>
            </div>
          )}
        </div>
      </div>
