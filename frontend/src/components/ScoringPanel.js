        ...
        <div className="p-5 flex-grow">
          {scores ? (
            <>
              ...
              <ScoreBar label="LLM Engagement Evaluator" score={scores.llm} color="bg-indigo-500" weight="0.3" />
              ...
              <div className="text-center">
                ...
                <div className="relative inline-flex items-center justify-center">
                  <svg className="w-32 h-32" viewBox="0 0 120 120">
                    ...
                    <circle
                      ...
                      r="50"
                      fill="none"
                      stroke={scores.final >= 0.75 ? '#1d4ed8' : '#d97706'}
                      strokeWidth="8"
                      strokeLinecap="round"
                ...
                <p className={`text-sm font-semibold mt-2 ${scores.final >= 0.75 ? 'text-emerald-700' : 'text-amber-700'}`}>
                  {scores.final >= 0.85
                    ? 'Excellent: ready to publish'
                    : scores.final >= 0.75
                      ? 'Good: passed quality gate'
                      : 'Below threshold: reflexion suggested'}
                </p>
              </div>
              ...
              <div className="mt-6 bg-slate-50 rounded-xl p-4 border border-slate-100">
                ...
                <p className="text-xs text-slate-600 font-mono leading-loose">
                  (0.5 x {(scores.ml * 100).toFixed(1)}) + (0.3 x {(scores.llm * 100).toFixed(1)}) + (0.2 x {(scores.heuristic * 100).toFixed(1)})<br />
              ...
              {scores.ml_top_importances && Object.keys(scores.ml_top_importances).length > 0 && (
                <div className="mt-4 bg-blue-50/70 rounded-xl p-4 border border-blue-100">
                  <p className="text-[11px] font-bold text-blue-600 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    <BarChart3 className="w-3 h-3" /> Top ML Feature Signals
                  </p>
                      ...
                      return (
                        <div key={feature}>
                          <div className="flex justify-between items-baseline mb-0.5">
                            <span className="text-[11px] text-slate-600 font-medium">{feature.replace(/_/g, ' ')}</span>
            ...
            <div className="flex flex-col items-center justify-center h-full text-slate-300 gap-3 py-12">
              ...
              <p className="text-xs text-slate-300">Waiting for generation to complete...</p>
            </div>
          )}
        </div>
      </div>
