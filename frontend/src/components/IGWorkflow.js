...
import { BrainCircuit, Search, Shield, AlertTriangle, Activity, Hash, RefreshCw, CheckCircle, Loader2 } from 'lucide-react';
      ...
      <div className="flex flex-col items-center">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${
          isDone
            ...
            : isActive
              ? 'bg-fuchsia-500 border-fuchsia-400 text-white animate-pulse'
              : 'bg-white border-slate-200 text-slate-400'
        }`}>
          {isDone ? <CheckCircle className="w-4 h-4" /> : isActive ? <Loader2 className="w-4 h-4 animate-spin" /> : <Icon className="w-4 h-4" />}
        </div>
        {!isLast && <div className={`w-0.5 flex-grow min-h-[24px] transition-colors ${isDone ? 'bg-pink-200' : 'bg-slate-100'}`} />}
      </div>
      <div className="pb-5 min-w-0">
        <p className={`text-sm font-semibold truncate ${isDone ? 'text-slate-800' : isActive ? 'text-fuchsia-700' : 'text-slate-400'}`}>
          {step.label}
        </p>
        <p className={`text-xs mt-0.5 leading-relaxed line-clamp-2 ${isDone ? 'text-slate-500' : isActive ? 'text-fuchsia-500' : 'text-slate-300'}`}>
          {detail || step.desc}
        </p>
      </div>
    </div>
  ...
  return (
    <div className="lg:col-span-4 space-y-4">
      <div className="ui-card overflow-hidden">
        <div className="ui-card-header bg-pink-50/70">
          <h2 className="font-semibold text-sm flex items-center gap-2 text-slate-800">
            <BrainCircuit className="w-4 h-4 text-pink-600" /> Instagram Agent Pipeline
          </h2>
          {loading && (
              ...
              <div key={i} className={`flex items-start gap-2 border rounded-lg px-3 py-2 ${r.manual ? 'bg-blue-50 border-blue-200' : 'bg-amber-50 border-amber-200'}`}>
                ...
                <div>
                  <p className={`text-xs font-semibold ${r.manual ? 'text-blue-700' : 'text-amber-700'}`}>
                    Iteration {r.iteration}: {r.manual ? 'User refinement' : `Score ${(r.previous_score * 100).toFixed(1)}% below threshold`}
                  </p>
