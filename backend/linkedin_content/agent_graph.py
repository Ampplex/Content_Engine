...
from competitor_analysis import competitor_analysis_node  # ← NEW
...
def _msg_text(content: Any) -> str:
    """Normalize LangChain message content to plain text for safe string ops."""
...
class _RequiredPostState(TypedDict):
    topic: str
    target_language: str

class PostState(_RequiredPostState, total=False):
    english_draft: str
    localized_draft: str
    ...
    # ── New fields ──────────────────────────────────
    competitor_insights: dict          # structured competitor intelligence
    competitor_context: str            # formatted string injected into draft prompts
    ab_variants: List[dict]            # 2-3 post variants with scores
    ab_variant_hooks: List[str]        # hook for each variant
...
def generate_base_draft(state: PostState):
    ...
    competitor_ctx = state.get("competitor_context", "")
...
- Make every sentence add value — be specific, not vague."""
    ...
    draft_text = _msg_text(response.content)
    logger.info(f"===== [NODE: drafting] Done. Draft length: {len(draft_text)} chars")
    return {"english_draft": draft_text, "iteration": iteration}

def trend_search_agent(state: PostState):
    """Generate search queries, perform web searches, fact-check, and enrich the draft."""
    topic = state['topic']
    draft = state.get('english_draft', '')
...
<the improved post>"""
    ...
    content = _msg_text(response.content).strip()
...
Output ONLY the hook text. Nothing else."""
    ...
    response = llm.invoke(prompt)
    hook = _msg_text(response.content).strip().strip('"').strip("'")
...
def indic_localization_agent(state: PostState):
    ...
    hook = state.get('hook', '')
    english_draft = state.get('english_draft', '')
...
def multi_agent_critique(state: PostState):
    ...
    draft = state.get('localized_draft', '')
    topic = state['topic']
    lang = state.get('target_language', 'English')
...
def hybrid_scoring_engine(state: PostState):
    ...
    text = state.get('localized_draft', '')
    ...
    try:
        ...
        llm_score = float(_msg_text(llm_eval.content).strip())
...
Return ONLY the image generation prompt, nothing else. No quotes, no explanation."""
    ...
    response = llm.invoke(prompt)
    image_prompt = _msg_text(response.content).strip().strip('"').strip("'")
...
def ab_variants_generator(state: PostState):
    """
    Generate 2 alternative post variants using different angles/tones,
    score each with LightGBM, and return them alongside the main post.

    Variants are generated AFTER the full critique pipeline so each one
    benefits from competitor intelligence and trend data.
    """
    topic    = state["topic"]
    lang     = state.get("target_language", "English")
    main     = state.get("localized_draft", "")
    ctx      = state.get("competitor_context", "")
    insights = state.get("trend_insights", "")
    iteration = state.get("iteration", 0)

    logger.info(f"===== [NODE: ab_variants] Generating 2 alternate variants | Iter: {iteration}")

    lang_instruction = f"Write the post in {lang}." if lang != "English" else ""
    context_block    = f"\n{ctx}\n" if ctx else ""
    trend_block      = f"\nRecent trend insights: {insights[:300]}\n" if insights else ""

    VARIANT_STYLES = [
        {
            "label":       "Story-Led",
            "description": "Open with a personal story or vivid scenario. Use 'I' voice. "
                           "Make the reader feel something before delivering the insight.",
        },
        {
            "label":       "Data-First",
            "description": "Open with a striking statistic or counterintuitive data point. "
                           "Build the post around evidence and numbers. Professional, authoritative tone.",
        },
    ]
    ...
    for style in VARIANT_STYLES:
        variant_prompt = f"""Write a LinkedIn post about: "{topic}" using a {style['label']} approach.

Style guide: {style['description']}
{lang_instruction}
{context_block}{trend_block}
Rules:
- TARGET LENGTH: 1,200–1,800 characters
- This is a DIFFERENT ANGLE from the standard approach — be creative and distinctive
- Use **bold** for key terms/stats
- 1-3 sentence paragraphs with blank lines between them
- Include 3-5 bullet points for key takeaways
- End with a CTA or question, followed by 3-5 relevant hashtags
- NO meta-labels like "Post:" or "Hook:"
- Write ONLY the post body, no headline"""

        try:
            resp     = llm.invoke(variant_prompt)
            body     = _msg_text(resp.content).strip()

            # Generate a hook for this variant
            hook_prompt = f"""Write a 1-line scroll-stopping LinkedIn hook for this {style['label']} post.
Under 15 words. Bold claim or curiosity-gap. No emojis.
{lang_instruction}

Post: {body[:400]}

Output ONLY the hook. Nothing else."""
            hook_resp = llm.invoke(hook_prompt)
            hook      = _msg_text(hook_resp.content).strip().strip('"').strip("'")

            # Score with LightGBM
            try:
                ml_result = post_predictor.predict_with_details(body)
            ...
            variant_hooks.append(hook)
            logger.info(f"  -> Variant '{style['label']}' done. Score: {score:.4f}, Length: {len(body)}")

        except Exception as e:
            logger.error(f"  -> Variant '{style['label']}' failed: {e}")

    logger.info(f"===== [NODE: ab_variants] Done. {len(variants)} variants generated.")
    return {
        "ab_variants":      variants,
        "ab_variant_hooks": variant_hooks,
    }


# Compile Graph
workflow = StateGraph(PostState)
def _competitor_node(state: PostState) -> dict:
    """Typed wrapper so LangGraph's StateNode type check passes."""
    return competitor_analysis_node(state)

workflow.add_node("drafting",             generate_base_draft)
workflow.add_node("competitor_analysis",  _competitor_node)
...
workflow.add_edge("competitor_analysis", "trend_search")
...
def reflexion_router(state: PostState):
    score = state.get("hybrid_score", {}).get("final", 0.0)
    iteration = state.get("iteration", 0)
