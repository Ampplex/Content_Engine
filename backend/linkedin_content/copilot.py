...
No markdown, no explanation."""
    ...
    try:
        data = json.loads(_msg_text(response.content).strip().strip("```json").strip("```"))
...
def _generate_strategy(df: pd.DataFrame, ml_predictions: Optional[list] = None) -> str:
...
Provide your recommendation in 2-3 concise paragraphs. Be specific with numbers."""
    ...
    response = llm.invoke(prompt)
    return _msg_text(response.content).strip()
